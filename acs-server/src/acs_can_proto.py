...
class can_raw_sock(object):
    ...
    # raises OSError
    def send(self, can_id:int, data, flags=0):
        can_id = can_id | flags
        dlc = len(data)
...
class acs_can_proto(object):
    ...
    # ACS data for FC's
    PANEL_CTRL_DATA_DEF = [0x00]
    PANEL_CTRL_DATA_UNLCK = [0x01]
    PANEL_CTRL_DATA_LCK = [0x02]
    PANEL_CTRL_DATA_LEARN = [0x03]
    PANEL_CTRL_DATA_CLR_CACHE = [0x04]
    DOOR_STATUS_DATA_CLOSED = [0x0]
    DOOR_STATUS_DATA_OPEN = [0x1]
    ...
    def __msg(self, prio, fc, dst, data_array):
        can_id = prio << self.ACS_PRIO_OFFSET
        can_id |= fc << self.ACS_FC_OFFSET
        can_id |= dst << self.ACS_DST_ADDR_OFFSET
        can_id |= self.addr << self.ACS_SRC_ADDR_OFFSET
        can_id |= socket.CAN_EFF_FLAG
        data = bytes(data_array)
        return (can_id, data)

    def msg_auth_fail(self, panel_addr, user_id):
        return self.__msg(self.PRIO_USER_AUTH_RESP_FAIL, self.FC_USER_AUTH_RESP_FAIL, panel_addr, user_id)

    def msg_auth_ok(self, panel_addr, user_id):
        return self.__msg(self.PRIO_USER_AUTH_RESP_OK, self.FC_USER_AUTH_RESP_OK, panel_addr, user_id)

    def msg_panel_normal(self, panel_addr):
        return self.__msg(self.PRIO_PANEL_CTRL, self.FC_PANEL_CTRL, panel_addr, self.PANEL_CTRL_DATA_DEF)

    def msg_panel_unlock(self, panel_addr):
        return self.__msg(self.PRIO_PANEL_CTRL, self.FC_PANEL_CTRL, panel_addr, self.PANEL_CTRL_DATA_UNLCK)

    def msg_panel_lock(self, panel_addr):
        return self.__msg(self.PRIO_PANEL_CTRL, self.FC_PANEL_CTRL, panel_addr, self.PANEL_CTRL_DATA_LCK)

    def msg_panel_learn(self, panel_addr):
        return self.__msg(self.PRIO_PANEL_CTRL, self.FC_PANEL_CTRL, panel_addr, self.PANEL_CTRL_DATA_LEARN)

    def msg_new_user(self, panel_addr, user_id):
        return self.__msg(self.PRIO_NEW_USER, self.FC_NEW_USER, panel_addr, user_id)
    ...
    def process_msg(self, msg_head:int, msg_len:int, msg_data):
        ...
        if (dst == self.addr or dst == self.ACS_BROADCAST_ADDR) and msg_head & socket.CAN_EFF_FLAG:
            if fc == self.FC_USER_AUTH_REQ and msg_len >= 4 and src > self.ACS_PNL_FIRST_ADDR:
                if self.cb_user_auth_req:
                    return self.cb_user_auth_req(src, int(msg_data[:4]))
            elif fc == self.FC_NEW_USER and msg_len >= 4 and src > self.ACS_PNL_FIRST_ADDR:
                if self.cb_new_user:
                    return self.cb_new_user(src, int(msg_data[:4]))
            else:
